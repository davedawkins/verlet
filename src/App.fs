...
let private _StickIds = IdFactory()
...
type Model =
    ...
    member __.Energy =
        __.AllParticles |> Seq.sumBy (fun p -> p.Energy)
    ...
    member __.AddParticle( xpos : float, ypos : float, ?vx : float, ?vy : float, ?isFixed : bool, ?isVisible : bool, ?gravity : bool, ?vertical : bool, ?color : string ) =
        __.AddParticle(Particle.Create(xpos, ypos, vx, vy, isFixed, isVisible, gravity, vertical, color))
    ...
    member __.GetParticle (id:int) =
        __.Particles[id]
    member __.FindParticle (id:int) =
        __.Particles.TryFind id


let makeParticles  (n:int) (model:Model) =
    let rp (m:Model) (i : int) =
        ...
        m.AddParticle( x, y, vx, vy, false, true, true, false, rndColor() )

    let shape = Shape.Empty
    [1..n] |> List.fold rp model

let makeBox (posx : float) (posy:float) (size : float) (vx : float) (vy : float) (model:Model) =
    let shape = Shape.Empty

    let firstId = Particle.ParticleIds.PeekNext()
...
let addChain (x1:float) (y1:float) (x2:float) (y2:float) (len:float) (numLinks:int) (fixing : ChainFixing) (model:Model) =
    let id0 = Particle.ParticleIds.PeekNext()
    let dx = (x2 - x1) / float numLinks
    let dy = (y2 - y1) / float numLinks
    let stickLen = len / float numLinks

    let addLink (m : Model) n =
        let isFixed =
            match fixing with
            | NoFixings -> false
            | FixAll -> true
...
let makeChain (x1:float) (y1:float) (x2:float) (y2:float) (len:float) (numLinks:int) (model : Model) =
    model |> addChain x1 y1 x2 y2 len numLinks FixEndsOnly

let makeNet (x1:float) (y1:float) (x2:float) (y2:float) (len:float) (numLinks:int) (height : float) (numHLinks : int) (model : Model) =
    let dh = height / float numHLinks

    let addVert (m : Model) n =
        m.AddStick(
            n + Particle.ParticleIds.PeekNext() - (numLinks+1),
            n + Particle.ParticleIds.PeekNext() - (numLinks+1) * 2,
            dh,
            true, true )
    ...
    let addChainRow (m : Model) (n : int) =
        m
        |> addChain x1 (y1 + float n * dh) x2 (y2 + float n * dh) len numLinks NoFixings
        |> addVerts

    [1..numHLinks-1]
    |> List.fold addChainRow (model |> addChain x1 y1 x2 y2 len numLinks FixEndsOnly)
...
let initEventHandlers (cvs : HTMLCanvasElement) (model : unit -> Model) (dispatch: Message -> unit) =
    ...
    cvs.onmousedown <- fun e ->
        let p =
            ...
            //|> List.concat
            |> Seq.tryFind (fun (p:Particle) -> p.Contains(e.offsetX,e.offsetY))
        match p with
        | None ->
            ()
        | Some x ->
...
let view (en : HTMLDivElement) (cvs : HTMLCanvasElement) ( m : Model ) =
    ...
    for p in m.Particles.Values |> Seq.filter (fun p -> p.Visible) do
        ctx.beginPath()
        ctx.fillStyle <-
    ...
    //for shape in m.Shapes do
    ctx.beginPath()
    for s in m.Sticks |> List.filter (fun s -> s.Visible) do
        let p1, p2 =
            m.Particles[s.P1],
            m.Particles[s.P2]
    ...
    let totalE = m.Energy
    en.innerText <- sprintf "%0.1f" totalE
    ...
    let collideP1 (e: float) (p1:Particle) (p2:Particle) =
        ...
        let n =
            if dx = 0 && dy = 0 then
                ...
                - (Vector2(dx,dy).Normalize()) // From p2 to p1
        ...
        let log() =
            if loggingEnabled then
                ...
                console.log("v1", v1)
                console.log("v2", v2)
                console.log("v1_speed_contrib", v1_speed_contrib)
        ...
        if
            (System.Double.IsNaN(v1_speed_contrib)) ||
            (System.Double.IsNaN(v2_speed_contrib))
        then log(); p1 else
        {
            p1 with
                XPos = c.X
...
//        Shapes = m.Shapes |> List.map updateShape
        Particles =
            m.Particles
            |> updateParticles
            |> applyConstraints 50 (m.Sticks)
    }

let pin (model:Model) =
    let (id,x,y) = model.Pinned
    match (model.FindParticle(id)) with
    | None -> model
    | Some p ->
        let p' =
            { p with
